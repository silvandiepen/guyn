...
export const getName = (color: HEX) => {
  ...
  const cleanUpHex = color.toUpperCase().replace("#", "");

  // return cleanUpHex;
  for (let i = 0; i < names.length; i++) {
    if (names[i][0] === cleanUpHex) {
      return names[i][1];
    }
  }

  return nearestColor(color)[1];
};
...
const nearestColor = (color: HEX) => {
  let lowest = Number.POSITIVE_INFINITY;
  let tmp;
  let index = 0;

  names.forEach((el, i) => {
    tmp = distance(hexToRgb(color), hexToRgb(el[0]));
    if (tmp < lowest) {
      lowest = tmp;
      index = i;
    }
